using System;
using System.Collections.Generic;
using System.IO.Ports;
using System.Text;

namespace Zhaoxi.Test
{
    class Program
    {
        static void Main(string[] args)
        {
            Console.WriteLine("Hello World!");

            // OPC   STRING10
            string text = "Hello 中国，朝夕教育";
            // 48 65 6C 6C 6F 20 E4 B8 AD E5 9B BD
            //List<byte> value = new List<byte> { 0x48, 0x65, 0x6C, 0x6C, 0x6F, 0x20, 0xE4, 0xB8, 0xAD, 0xE5, 0x9B, 0xBD };
            //text = Encoding.UTF8.GetString(value.ToArray());
            List<byte> textBytes = new List<byte>(Encoding.UTF8.GetBytes(text));
            if (textBytes.Count % 2 > 0)
                textBytes.Add(0x00);

            int count = textBytes.Count / 2;
            //List<byte> bytes = new List<byte>{
            //    0x01,0x10,
            //    0x00,0x01,
            //    BitConverter.GetBytes(count)[1],
            //    BitConverter.GetBytes(count)[0],
            //    (byte)textBytes.Count
            //};
            //bytes.AddRange(textBytes);

            List<byte> bytes = new List<byte>{
                0x01,0x03,
                0x00,0x01,
                0x00,0x06
            };
            bytes.AddRange(Crc16(bytes.ToArray(), bytes.Count));

            SerialPort serialPort = new SerialPort();
            serialPort.Open();
            serialPort.Write(bytes.ToArray(), 0, bytes.Count);



        }




        #region  CRC校验
        private static readonly byte[] aucCRCHi = {
             
         };
        private static readonly byte[] aucCRCLo = {
             
         };
        private static byte[] Crc16(byte[] pucFrame, int usLen)
        {
            int i = 0;
            byte crcHi = 0xFF;
            byte crcLo = 0xFF;
            UInt16 iIndex = 0x0000;

            while (usLen-- > 0)
            {
                iIndex = (UInt16)(crcLo ^ pucFrame[i++]);
                crcLo = (byte)(crcHi ^ aucCRCHi[iIndex]);
                crcHi = aucCRCLo[iIndex];
            }

            return new byte[] { crcLo, crcHi };
        }

        #endregion
    }
}
